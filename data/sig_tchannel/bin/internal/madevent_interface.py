...
class CmdExtended(common_run.CommonRunCmd):
    ...
    def stop_on_keyboard_stop(self):
        ...
        try:
            ...
            if hasattr(self, "results"):
                self.update_status(
                    "Stop by the user", level=None, makehtml=False, error=True
                )
    ...
    def postcmd(self, stop, line):
        ...
        if len(arg) == 0:
            return stop
        if isinstance(self.results.status, str) and self.results.status.startswith(
            "Error"
        ):
            return stop
        if (
            isinstance(self.results.status, str)
            and self.results.status == "Stop by the user"
        ):
        ...
        try:
            self.update_status(
                "Command '%s' done.<br> Waiting for instruction." % arg[0],
                level=None,
                error=True,
            )
        except Exception:
        ...
        if isinstance(error, ZeroResult):
            ...
            try:
                ...
                if not os.path.isdir(out_dir):
                    os.mkdir(out_dir)
                output_path = pjoin(
                    out_dir, "%s_%s_banner.txt" % (self.run_name, self.run_tag)
                )
...
class HelpToCmd(object):
    ...
    def run_options_help(self, data):
        ...
        logger.info(
            "      --cluster : Submit to the  cluster. Current cluster: %s"
    ...
    def help_generate_events(self):
        ...
        self.run_options_help(
            [
                ...
                (
                    "--laststep=",
                    "argument might be parton/pythia/pgs/delphes and indicate the last level to be run.",
                ),
   ...
   parameters can have affected these filters, this command allows you to automatically refresh them. """
        ...
        logger.info(
            "     -f          : Bypass the edition of MadLoopParams.dat.",
            "$MG:color:BLUE",
        )
        logger.info(
            "     -r          : Refresh of the existing filters (erasing them if already present).",
            "$MG:color:BLUE",
        )
        logger.info(
            "     --nPS=<int> : Specify how many phase-space points should be tried to set up the filters.",
            "$MG:color:BLUE",
        )
    ...
    def help_add_time_of_flight(self):
        ...
        logger.info(
            "   threshold option allows to change the minimal value required to"
        )
    ...
    def help_calculate_decay_widths(self):
        ...
        self.run_options_help(
            [
                ...
                (
                    "--accuracy=",
                    "accuracy (for each partial decay width)." + " Default is 0.01.",
                ),
            ]
        )
    ...
    def help_survey(self):
        ...
        self.run_options_help(
            [("--" + key, value[-1]) for (key, value) in self._survey_options.items()]
        )
    ...
    def help_launch(self):
        ...
        if self.ninitial == 1:
            logger.info(
                "For this directory this is equivalent to calculate_decay_widths"
            )
    ...
    def help_combine_events(self):
        """ """
        logger.info(
            "syntax: combine_events [run_name] [--tag=tag_name] [--run_options]"
        )
    ...
    def help_syscalc(self):
        logger.info(
            "syntax: syscalc [RUN] [%s] [-f | --tag=]" % "|".join(self._plot_mode)
        )
        logger.info(
            "-- calculate systematics information for the RUN (current run by default)"
        )
        logger.info(
            "     at different stages of the event generation for scale/pdf/..."
        )
...
class AskRun(cmd.ControlSwitch):
    ...
    def __init__(self, question, line_args=[], mode=None, force=False, *args, **opt):

        self.check_available_module(opt["mother_interface"].options)
        self.me_dir = opt["mother_interface"].me_dir
        super(AskRun, self).__init__(
            self.to_control, opt["mother_interface"], *args, **opt
        )
    ...
    def set_default_shower(self):
        ...
        if "PY6" in self.available_module and os.path.exists(
            pjoin(self.me_dir, "Cards", "pythia_card.dat")
        ):
            self.switch["shower"] = "Pythia6"
        elif "PY8" in self.available_module and os.path.exists(
            pjoin(self.me_dir, "Cards", "pythia8_card.dat")
        ):
    ...
    def set_default_detector(self):
        ...
        if (
            "PGS" in self.available_module
            and self.switch["shower"] == "Pythia6"
            and os.path.exists(pjoin(self.me_dir, "Cards", "pgs_card.dat"))
        ):
            self.switch["detector"] = "PGS"
        elif (
            "Delphes" in self.available_module
            and self.switch["shower"] != "OFF"
    ...
    def set_default_analysis(self):
        ...
        if "MA4" in self.available_module and os.path.exists(
            pjoin(self.me_dir, "Cards", "plot_card.dat")
        ):
            ...
            or os.path.exists(
                pjoin(self.me_dir, "Cards", "madanalysis5_hadron_card.dat")
            )
        ):
    ...
    def get_cardcmd_for_madspin(self, value):
        ...
        if value == "onshell":
            return [
                "edit madspin_card --replace_line='set spinmode' --before_line='decay' set spinmode onshell"
            ]
        elif value in ["full", "madspin"]:
            return [
                "edit madspin_card --replace_line='set spinmode' --before_line='decay' set spinmode madspin"
            ]
        elif value == "none":
            return [
                "edit madspin_card --replace_line='set spinmode' --before_line='decay' set spinmode none"
            ]
        else:
            return []
...
class CheckValidForCmd(object):
    ...
    def check_banner_run(self, args):
        ...
        if os.path.exists(args[0]):
            ...
            args[0] = pjoin(
                self.me_dir, "Events", args[0], "%s_%s_banner.txt" % (args[0], tag)
            )
        ...
        if run_name:
            ...
            if not self.results[name].is_empty():
                ...
                logger.info(
                    "Run %s is not empty so will use run_name: %s" % (name, run_name)
                )
    ...
    def check_history(self, args):
        ...
        if not len(args):
            ...
            if dirpath and not os.path.exists(dirpath) or os.path.isdir(args[0]):
    ...
    def check_save(self, args):
        ...
        if args[0] != "options" and len(args) != 2:
            ...
            if not os.path.exists(basename):
                raise self.InvalidCmd("%s is not a valid path, please retry" % args[1])
                ...
                if arg in ["--auto", "--all"]:
                    ...
                    if not os.path.exists(basename):
                        raise self.InvalidCmd(
                            "%s is not a valid path, please retry" % arg
                        )
                    elif has_path:
            ...
            if not has_path:
                if "--auto" in arg and self.options["mg5_path"]:
                    args.insert(
                        1,
                        pjoin(
                            self.options["mg5_path"], "input", "mg5_configuration.txt"
                        ),
                    )
                else:
    ...
    def check_set(self, args):
        ...
        if args[0] not in self._set_options + self.options.keys():
            self.help_set()
            raise self.InvalidCmd("Possible options for set are %s" % self._set_options)

        if args[0] in ["stdout_level"]:
            if (
                args[1] not in ["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"]
                and not args[1].isdigit()
            ):
                raise self.InvalidCmd("output_level needs " + "a valid level")
    ...
    def check_open(self, args):
        ...
        if not self.me_dir:
            if not os.path.isfile(args[0]):
                self.help_open()
                raise self.InvalidCmd(
                    "No MadEvent path defined. Unable to associate this name to a file"
                )
            else:
                return True
        ...
        # special for card with _default define: copy the default and open it
            ...
            if os.path.isfile(os.path.join(path, "Cards", name)):
                files.cp(
                    os.path.join(path, "Cards", name),
                    os.path.join(path, "Cards", args[0]),
                )
                args[0] = os.path.join(path, "Cards", args[0])
            else:
    ...
    def check_treatcards(self, args):
        ...
        opt = {
            "output_dir": pjoin(self.me_dir, "Source"),
        ...
        for arg in args:
            if arg.startswith("--") and "=" in arg:
                ...
                if not key in opt:
                    ...
                    raise self.InvalidCmd(
                        "Invalid option for treatcards command:%s " % key
                    )
                if key in ["param_card", "run_card"]:
                    if os.path.isfile(value):
                        ...
                        if card_name != key:
                            raise self.InvalidCmd(
                                "Format for input file detected as %s while expecting %s"
                                % (card_name, key)
                            )
                        opt[key] = value
            ...
            try:
                for opt, value in self._survey_options.items():
                    if arg.startswith("--%s=" % opt):
                        exec (
                            "self.opts['%s'] = %s(arg.split('=')[-1])" % (opt, value[0])
                        )
                        arg = ""
                if arg != "":
                    raise Exception
    ...
    def check_generate_events(self, args):
        ...
        if args and args[-1].startswith("--laststep="):
            ...
            if run != "parton" and not self.options["pythia-pgs_path"]:
                raise self.InvalidCmd(
                    """pythia-pgs not install. Please install this package first. 
                To do so type: \'install pythia-pgs\' in the mg5 interface"""
                )
            if run == "delphes" and not self.options["delphes_path"]:
                raise self.InvalidCmd(
                    """delphes not install. Please install this package first. 
    ...
    def check_add_time_of_flight(self, args):
        ...
        if len(args) == 1 and os.path.exists(args[0]):
            ...
            event_path = pjoin(
                self.me_dir, "Events", self.run_name, "unweighted_events.lhe.gz"
            )
    ...
    def check_calculate_decay_widths(self, args):
        ...
        if self.ninitial != 1:
            raise self.InvalidCmd(
                "Can only calculate decay widths for decay processes A > B C ..."
            )
        ...
        if args and args[-1].startswith("--accuracy="):
            try:
                ...
                raise self.InvalidCmd(
                    "Argument error in calculate_decay_widths command"
                )
            del args[-1]
        if len(args) > 1:
            self.help_calculate_decay_widths()
            raise self.InvalidCmd(
                "Too many argument for calculate_decay_widths command: %s" % cmd
            )
    ...
    def check_multi_run(self, args):
        ...
        if not len(args):
            self.help_multi_run()
            raise self.InvalidCmd(
                """multi_run command requires at least one argument for
        ...
        if args[-1].startswith("--laststep="):
            ...
            self.help_multi_run()
            raise self.InvalidCmd(
                "The first argument of multi_run should be a integer."
            )
    ...
    def check_refine(self, args):
        ...
        try:
            ...
            self.help_refine()
            raise self.InvalidCmd(
                "require_precision argument is require for refine cmd"
            )
        ...
        if not self.run_name:
            if self.results.lastrun:
                ...
                raise self.InvalidCmd(
                    "No run_name currently define. Unable to run refine"
                )
    ...
    def check_combine_events(self, arg):
        ...
        if not self.run_name:
            if not self.results.lastrun:
                raise self.InvalidCmd(
                    "No run_name currently define. Unable to run combine"
                )
            else:
                self.set_run_name(self.results.lastrun)
    ...
    def check_pythia(self, args):
        ...
        if not self.options["pythia-pgs_path"] or not os.path.exists(
            pjoin(self.options["pythia-pgs_path"], "src")
        ):
        ...
        if len(args) == 0 and not self.run_name:
            if self.results.lastrun:
                ...
                raise self.InvalidCmd(
                    "No run name currently define. Please add this information."
                )

        if len(args) >= 1:
            if args[0] != self.run_name and not os.path.exists(
                pjoin(self.me_dir, "Events", args[0], "unweighted_events.lhe.gz")
            ):
                raise self.InvalidCmd(
                    "No events file corresponding to %s run. " % args[0]
                )
        ...
        input_file = pjoin(
            self.me_dir, "Events", self.run_name, "unweighted_events.lhe"
        )
        output_file = pjoin(self.me_dir, "Events", "unweighted_events.lhe")
        if not os.path.exists("%s.gz" % input_file):
            if not os.path.exists(input_file):
                raise self.InvalidCmd(
                    "No events file corresponding to %s run. " % self.run_name
                )
    ...
    def check_pythia8(self, args):
        ...
        if not self.options["pythia8_path"] or not os.path.exists(
            pjoin(self.options["pythia8_path"], "bin", "pythia8-config")
        ):
        ...
        if not os.path.exists("%s.gz" % input_file):
            if os.path.exists(input_file):
                ...
                raise self.InvalidCmd(
                    "No event file corresponding to %s run. " % self.run_name
                )
        ...
        if len(tmp_args) == 0:
            ...
            for arg in tmp_args[1:]:
                if arg not in self._clean_mode:
                    self.help_remove()
                    raise self.InvalidCmd(
                        "%s is not a valid options for clean command" % arg
                    )
    ...
    def check_syscalc(self, args):
        ...
        if len(args) == 0:
            if not hasattr(self, "run_name") or not self.run_name:
                self.help_syscalc()
                raise self.InvalidCmd(
                    "No run name currently defined. Please add this information."
                )
            args.append("all")
            return
        ...
        if self.run_card["use_syst"] not in self.true:
            raise self.InvalidCmd(
                "Run %s does not include " % self.run_name
    ...
    def check_pgs(self, arg, no_default=False):
        ...
        if not len(arg) and not os.path.exists(
        ...
        if len(arg) == 1:
            ...
            if not os.path.exists(
                pjoin(
                    ...
                    "Events",
                    self.run_name,
                    "%s_pythia_events.hep.gz" % prev_tag,
                )
            ):
                raise self.InvalidCmd(
                    "No events file corresponding to %s run with tag %s. "
                ...
                lock = cluster.asyncrone_launch(
                    "gunzip", stdout=open(output_file, "w"), argument=["-c", input_file]
                )
...
class CompleteForCmd(CheckValidForCmd):
    ...
    def complete_add_time_of_flight(self, text, line, begidx, endidx):
        ...
        if len(args) == 1:
            ...
            data = misc.glob(
                pjoin("*", "unweighted_events.lhe.gz"), pjoin(self.me_dir, "Events")
            )
            data = [n.rsplit("/", 2)[1] for n in data]
            return self.list_completion(text, data + ["--threshold="], line)
        elif args[-1].endswith(os.path.sep):
            return self.path_completion(
                text, os.path.join(".", *[a for a in args if a.endswith(os.path.sep)])
            )
        else:
    ...
    def complete_banner_run(self, text, line, begidx, endidx, formatting=True):
        ...
        try:
            ...
            if len(args) > 1:
                ...
                tags = misc.glob(
                    "%s_*_banner.txt" % args[1], pjoin(self.me_dir, "Events", args[1])
                )
                tags = [
                    "%s" % os.path.basename(t)[len(args[1]) + 1 : -11] for t in tags
                ]
            ...
            run_list = misc.glob(
                pjoin("*", "*_banner.txt"), pjoin(self.me_dir, "Events")
            )
    ...
    def complete_open(self, text, line, begidx, endidx):
        ...
        if self.me_dir:
            ...
            if os.path.isdir(os.path.join(path, "Cards")):
                possibility += [
                    f
                    for f in os.listdir(os.path.join(path, "Cards"))
                    if f.endswith(".dat")
                ]
            if os.path.isdir(os.path.join(path, "HTML")):
                possibility += [
                    f
                    for f in os.listdir(os.path.join(path, "HTML"))
    ...
    def complete_set(self, text, line, begidx, endidx):
        ...
        if len(args) == 2:
            if args[1] == "stdout_level":
                ...
                # directory names
                second_set = [
                    name for name in self.path_completion(text, ".", only_dirs=True)
                ]
    ...
    def complete_generate_events(self, text, line, begidx, endidx):
        ...
        if line.endswith("laststep=") and not text:
            ...
            return self.list_completion(
                text, ["parton", "pythia", "pgs", "delphes"], line
            )
    ...
    def complete_initMadLoop(self, text, line, begidx, endidx):
        ...
        if len(line) >= 6 and line[begidx - 6 : begidx] == "--nPS=":
            ...
            return self.list_completion(
                text, [opt for opt in opts if not opt in line], line
            )
    ...
    def complete_display(self, text, line, begidx, endidx):
        ...
        if len(args) >= 2 and args[1] == "results":
            ...
            return self.complete_print_results(
                text,
                "print_results " + line[start + 7 :],
                begidx + 2 + start,
                endidx + 2 + start,
            )
    ...
    def complete_pythia8(self, text, line, begidx, endidx):
        ...
        if len(args) == 1:
            ...
            if not self.run_name:
                ...
                tmp2 = self.list_completion(
                    text, self._run_options + ["-f", "--no_default", "--tag="], line
                )
                return tmp1 + tmp2
        elif line[-1] != "=":
            return self.list_completion(
    ...
    def complete_madanalysis5_parton(self, text, line, begidx, endidx):
        ...
        if len(args) == 1:
            ...
            for name in ["unweighted_events.lhe"]:
                ...
                data += misc.glob(
                    pjoin("*", "%s.gz" % name), pjoin(self.me_dir, "Events")
                )
            data = [n.rsplit("/", 2)[1] for n in data]
            tmp1 = self.list_completion(text, data)
            if not self.run_name:
                ...
                tmp2 = self.list_completion(
                    text, ["-f", "--MA5_stdout_lvl=", "--no_default", "--tag="], line
                )
                return tmp1 + tmp2
        elif "--MA5_stdout_lvl=" in line and not any(
            arg.startswith("--MA5_stdout_lvl=") for arg in args
        ):
            return self.list_completion(
                text,
                [
                    "--MA5_stdout_lvl=%s" % opt
                    for opt in [
                        "logging.INFO",
                        "logging.DEBUG",
                        "logging.WARNING",
    ...
    def complete_pgs(self, text, line, begidx, endidx):
        ...
        if len(args) == 1:
            # return valid run_name
            data = misc.glob(
                pjoin("*", "*_pythia_events.hep.gz"), pjoin(self.me_dir, "Events")
            )
            data = [n.rsplit("/", 2)[1] for n in data]
            tmp1 = self.list_completion(text, data)
            if not self.run_name:
                ...
                tmp2 = self.list_completion(
                    text, self._run_options + ["-f", "--tag=", "--no_default"], line
                )
                return tmp1 + tmp2
        else:
            return self.list_completion(
...
class MadEventCmd(CompleteForCmd, CmdExtended, HelpToCmd, common_run.CommonRunCmd):
    ...
    def set_configuration(self, amcatnlo=False, final=True, **opt):
        ...
        super(MadEventCmd, self).set_configuration(
            amcatnlo=amcatnlo, final=final, **opt
        )
        ...
        for key in (k for k in self.options if k.endswith("path")):
            ...
            if os.path.isdir(path):
                ...
                if key == "pythia-pgs_path":
                    ...
                    if not os.path.exists(
                        pjoin(path, "Delphes")
                    ) and not os.path.exists(pjoin(path, "DelphesSTDHEP")):
    ...
    def do_banner_run(self, line):
        ...
        self.exec_cmd(
            "generate_events %s %s" % (self.run_name, self.force and "-f" or "")
        )
        ...
        if args[0] == "options":
            ...
            if "--all" in args:
                ...
                for key, default in self.options_madgraph.items():
                    if self.options[key] != self.options_madgraph[key]:
                        logger.info(
                            "The option %s is modified [%s] but will not be written in the configuration files."
...
Beware that MG5aMC now changes your runtime options to a multi-core mode with only one active core."""
        ...
        if self.run_card["gridpack"] in self.true:
            ...
            self.exec_cmd(
                ...
                % (
                    self.run_name,
                    " ".join(
                        ["--" + opt + "=" + str(val) for (opt, val) in gridpack_opts]
                    ),
                ),
                postcmd=False,
            )
            ...
            # Regular run mode
            logger.info(
                "Generating %s events with run name %s"
                % (self.run_card["nevents"], self.run_name)
            )

            self.exec_cmd(
                "survey  %s %s" % (self.run_name, " ".join(args)), postcmd=False
            )
   ...
   Please check/correct your param_card and/or your run_card."""
                ...
                if not self.param_card_iterator:
                    raise ZeroResult(
                        "See https://cp3.irmp.ucl.ac.be/projects/madgraph/wiki/FAQ-General-14"
                    )
                else:
                    bypass_run = True
            ...
            if not bypass_run:
                ...
                if self.run_card["use_syst"]:
                    if self.run_card["systematics_program"] == "auto":
                        ...
                        if not scdir or not os.path.exists(scdir):
                            ...
                            to_use = "syscalc"
                    elif self.run_card["systematics_program"].lower() in [
                        "systematics",
                        "syscalc",
                        "none",
                    ]:
                        ...
                        logger.critical(
                            "Unvalid options for systematics_program: bypass computation of systematics variations."
                        )
                        to_use = "none"
                    ...
                    if to_use == "systematics":
                        if self.run_card["systematics_arguments"] != [""]:
                            ...
                            self.exec_cmd(
                                "systematics %s --from_card" % self.run_name,
                                postcmd=False,
                                printcmd=False,
                            )
                    elif to_use == "syscalc":
                ...
                self.exec_cmd("store_events", postcmd=False)
                if (
                    self.run_card["boost_event"].strip()
                    and self.run_card["boost_event"] != "False"
                ):
                    self.boost_events()
                ...
                if self.run_card["time_of_flight"] >= 0:
                    self.exec_cmd(
                        "add_time_of_flight --threshold=%s"
                ...
                if switch_mode["analysis"] == "ExRoot":
                    ...
                    output = pjoin(
                        self.me_dir, "Events", self.run_name, "unweighted_events.root"
                    )
                    self.create_root_file(input, output)

                self.exec_cmd(
                    "madanalysis5_parton --no_default", postcmd=False, printcmd=False
                )
                # shower launches pgs/delphes if needed
                self.exec_cmd("shower --no_default", postcmd=False, printcmd=False)
                self.exec_cmd(
                    "madanalysis5_hadron --no_default", postcmd=False, printcmd=False
                )
                self.store_result()

            if self.allow_notification_center:
                misc.apple_notify(
                    "Run %s finished" % os.path.basename(self.me_dir),
                    "%s: %s +- %s "
                    % (
                        self.results.current["run_name"],
    ...
    def boost_events(self):
        ...
        with misc.TMP_directory() as tmp_dir:
            output = lhe_parser.EventFile(
                pjoin(tmp_dir, os.path.basename(event_path)), "w"
            )
            # write the banner to the output file
            output.write(lhe.banner)
            # Loop over all events
    ...
    def do_initMadLoop(self, line):
        ...
        if options["refresh"]:
            for filter in misc.glob(
                "*Filter*", pjoin(self.me_dir, "SubProcesses", "MadLoop5_resources")
            ):
                logger.debug("Resetting filter '%s'." % os.path.basename(filter))
                os.remove(filter)

        MLCard = banner_mod.MadLoopParam(
            pjoin(self.me_dir, "Cards", "MadLoopParams.dat")
        )
        if options["nPS"] is None:
            ...
            logger.debug(
                "Hard-setting user-defined n_PS (%d) to %d, because "
                % (options["nPS"], new_n_PS)
                + "of the 'CheckCycle' value (%d) " % MLCard["CheckCycle"]
                + "specified in the ML param card."
            )
            options["nPS"] = new_n_PS

        MadLoopInitializer.init_MadLoop(
            self.me_dir,
            n_PS=options["nPS"],
            subproc_prefix="PV",
            MG_options=self.options,
            interface=self,
        )
    ...
    def print_results_in_shell(self, data):
        ...
        if data["run_statistics"]:
            ...
            for key, value in data["run_statistics"].items():
                ...
                logger.log(
                    level,
                    value.nice_output(str("/".join([key[0], "G%s" % key[1]]))).replace(
                        " statistics", ""
                    ),
                )
            logger.info(" ")
        ...
        logger.info(
            "  === Results Summary for run: %s tag: %s ===\n"
            % (data["run_name"], data["tag"])
        )

        total_time = int(
            sum(_["cumulative_timing"] for _ in data["run_statistics"].values())
        )
        if total_time > 0:
            logger.info(
                "     Cumulative sequential time for this run: %s"
                % misc.format_time(total_time)
            )

        if self.ninitial == 1:
            logger.info(
                "     Width :   %.4g +- %.4g GeV" % (data["cross"], data["error"])
            )
        else:
            logger.info(
                "     Cross-section :   %.4g +- %.4g pb"
            ...
            if data["cross_pythia"] and data["nb_event_pythia"]:
                if data["cross_pythia"] == -1:
                    path = pjoin(
                        ...
                        "Events",
                        self.run_name,
                        "%s_merged_xsecs.txt" % self.run_tag,
                    )
                    cross_sections = {}
                    ...
                    if len(cross_sections) > 0:
                        ...
                        for scale in sorted(cross_sections.keys()):
                            logger.info(
                                "      > Merging scale = %-6.4g : %-11.5g +/- %-7.2g [pb]"
                                % (
                                    scale,
                                    cross_sections[scale][0],
                                    cross_sections[scale][1],
                                )
                            )
                    ...
                    if self.ninitial == 1:
                        logger.info(
                            "     Matched width :   %.4g +- %.4g GeV"
                            % (data["cross_pythia"], data["error_pythia"])
                        )
                    else:
                        logger.info(
                            "     Matched cross-section :   %.4g +- %.4g pb"
                            % (data["cross_pythia"], data["error_pythia"])
                        )
                        logger.info(
                            "     Nb of events after matching/merging :  %d"
                            % int(data["nb_event_pythia"])
                        )
                if self.run_card["use_syst"] in self.true and (
                    int(self.run_card["ickkw"]) == 1
                    or self.run_card["ktdurham"] > 0.0
    ...
    def print_results_in_file(self, data, path, mode="w", format="full"):
        ...
        if format == "full":
            fsock.write(
                "  === Results Summary for run: %s tag: %s  process: %s ===\n"
    ...
    def do_calculate_decay_widths(self, line):
        ...
        # Running gridpack warmup
        opts = [
            ("accuracy", accuracy),  # default 0.01
            ("points", 1000),
            ("iterations", 9),
        ]
        ...
        self.exec_cmd(
            ...
            % (
                self.run_name,
                " ".join(["--" + opt + "=" + str(val) for (opt, val) in opts]),
            ),
            postcmd=False,
        )
        ...
        self.print_results_in_shell(self.results.current)
        self.update_status(
            "calculate_decay_widths done", level="parton", makehtml=False
        )
    ...
    def collect_decay_widths(self):
        ...
        self.update_width_in_param_card(
            particle_dict,
    ...
    def update_width_in_param_card(decay_info, initial=None, output=None):
        ...
        for key in sorted(decay_info.keys()):
            ...
            if decay_info[key][0][0]:
                ...
                brs = [
                    [(val[1] / width).real, val[0]] for val in decay_info[key] if val[1]
                ]
    ...
    def do_multi_run(self, line):
        ...
        try:
            ...
            pass
        os.system(
            "%(bin)s/merge.pl %(event)s/%(name)s_*/unweighted_events.lhe.gz %(event)s/%(name)s/unweighted_events.lhe.gz %(event)s/%(name)s_banner.txt"
            % {
                "bin": self.dirbin,
                "event": pjoin(self.me_dir, "Events"),
                "name": self.run_name,
            }
        )
        ...
        if eradir and misc.is_executable(pjoin(eradir, "ExRootLHEFConverter")):
            ...
            misc.gunzip(
                "%s/%s/unweighted_events.lhe.gz"
                % (pjoin(self.me_dir, "Events"), self.run_name)
            )

            self.create_root_file(
                "%s/unweighted_events.lhe" % self.run_name,
        ...
        if param_card_iterator:
            ...
            param_card_iterator.store_entry(
                self.run_name, self.results.current["cross"], param_card_path=cpath
            )
    ...
    def do_treatcards(self, line, mode=None, opt=None):
        ...
        type_of_change = ""
        if (
            not opt["forbid_MadLoopInit"]
            and self.proc_characteristics["loop_induced"]
            and mode in ["loop", "all"]
        ):
            paramDat = pjoin(self.me_dir, "Cards", "param_card.dat")
            paramInc = pjoin(opt["output_dir"], "param_card.inc")
            if (
                (not os.path.isfile(paramDat))
                or (not os.path.isfile(paramInc))
                or (os.path.getmtime(paramDat) - os.path.getmtime(paramInc)) > 0.0
            ):
            ...
            ML_out = pjoin(
                self.me_dir, "SubProcesses", "MadLoop5_resources", "MadLoopParams.dat"
            )
            if (
                (not os.path.isfile(ML_in))
                or (not os.path.isfile(ML_out))
                or (os.path.getmtime(ML_in) - os.path.getmtime(ML_out)) > 0.0
            ):
                need_MadLoopFilterUpdate = True
        ...
        if mode in ["param", "all"]:
            ...
            if tmp_model == "mssm" or tmp_model.startswith("mssm-"):
                if not "--param_card=" in line:
                    ...
                    opt["param_card"] = pjoin(
                        self.me_dir, "Source", "MODEL", "MG5_param.dat"
                    )
            else:
            ...
            if os.path.isfile(
                pjoin(
                    self.me_dir, "bin", "internal", "ufomodel", "restrict_default.dat"
                )
            ):
                default = pjoin(
                    self.me_dir, "bin", "internal", "ufomodel", "restrict_default.dat"
                ...
                pjoin(self.me_dir, "bin", "internal", "ufomodel", "param_card.dat")
            ):
                default = pjoin(
                    self.me_dir, "bin", "internal", "ufomodel", "param_card.dat"
                )
                ...
                subprocess.call(
                    [sys.executable, "write_param_card.py"],
            ...
            if run_card["bias_module"].lower() not in ["dummy", "none"]:
                ...
                if not os.path.isdir(bias_module_path):
                    if not os.path.isdir(run_card["bias_module"]):
                        raise InvalidCmd(
                            "The bias module at '%s' cannot be found."
                            % run_card["bias_module"]
                        )
                    else:
                        for mandatory_file in [
                            "makefile",
                            "%s.f" % os.path.basename(run_card["bias_module"]),
                        ]:
                            if not os.path.isfile(
                                pjoin(run_card["bias_module"], mandatory_file)
                            ):
                                raise InvalidCmd(
                                    "Could not find the mandatory file '%s' in bias module '%s'."
                ...
                for line in open(
                    pjoin(bias_module_path, "%s.f" % os.path.basename(bias_module_path))
                ):
                    if start and last:
                        break
                    if not start and not re.search(
                        "c\s*parameters\s*=\s*{", line, re.I
                    ):
                        continue
                    start = True
                ...
                for key, value in run_card["bias_parameters"].items():
                    if key not in default_bias_parameters:
                        logger.warning(
                            "%s not supported by the bias module. We discard this entry.",
                            key,
                        )
                    else:
        ...
        if self.proc_characteristics["loop_induced"] and mode in ["loop", "all"]:
            self.MadLoopparam = banner_mod.MadLoopParam(
                pjoin(self.me_dir, "Cards", "MadLoopParams.dat")
            )
            ...
            if (
                "WriteOutFilters" in self.MadLoopparam.user_set
                and self.MadLoopparam.get("WriteOutFilters")
            ):
                logger.info(
...
Beware that this can be dangerous for local multicore runs."""
            ...
            self.MadLoopparam.set(
                "DoubleCheckHelicityFilter", False, changeifuserset=False
            )
            ...
            if run_card["nhel"] == 0:
                if "MLReductionLib" in self.MadLoopparam.user_set and (
                    ...
                    or self.MadLoopparam.get("MLReductionLib").startswith("6")
                ):
                    logger.warning(
    ...
    Beware that this can bring significant slowdown; the optimal choice --when not MC over helicity-- being to first start with TIR reduction."""
                ...
                if "MLReductionLib" in self.MadLoopparam.user_set and not (
                    self.MadLoopparam.get("MLReductionLib").startswith("1")
                    or self.MadLoopparam.get("MLReductionLib").startswith("6")
                ):
                    logger.warning(
    ...
    Beware that this can bring significant slowdown; the optimal choice --when MC over helicity-- being to first start with OPP reduction."""
            ...
            if run_card["nhel"] == 0:
                if (
                    "NRotations_DP" in self.MadLoopparam.user_set
                    and self.MadLoopparam.get("NRotations_DP") != 0
                ) or (
                    "NRotations_QP" in self.MadLoopparam.user_set
                    and self.MadLoopparam.get("NRotations_QP") != 0
                ):
                    logger.warning(
    ...
    the helicity of final state particles cannot be speicfied in the LHE file."""
            ...
            if self.proc_characteristics["nexternal"] <= 4:
                if (
                    "MLStabThres" in self.MadLoopparam.user_set
                    and self.MadLoopparam.get("MLStabThres") > 1.0e-7
                ):
                    logger.warning(
    ...
    than four external legs, so this is not recommended (especially not for g g > z z)."""
        ...
        if self.proc_characteristics["loop_induced"] and mode in ["loop", "all"]:
            ...
            if need_MadLoopFilterUpdate:
                logger.debug(
                    "Changes to the %s parameters" % type_of_change
            ...
            # be overwritten.
            elif not opt["forbid_MadLoopInit"] and MadLoopInitializer.need_MadLoopInit(
                self.me_dir
            ):
    ...
    def do_survey(self, line):
        ...
        self.total_jobs = 0
        subproc = [
            l.strip() for l in open(pjoin(self.me_dir, "SubProcesses", "subproc.mg"))
        ]
        ...
        if os.path.exists(
            ...
            tf = tarfile.open(
                pjoin(self.me_dir, "SubProcesses", "MadLoop5_resources.tar.gz"),
                "w:gz",
                dereference=True,
            )
            tf.add(
        ...
        if float(self.run_card["mmjj"]) > 0.01 * (
            float(self.run_card["ebeam1"]) + float(self.run_card["ebeam2"])
        ):
        ...
        if P_zero_result:
            if len(P_zero_result) == len(subproc):
                ...
                raise ZeroResult, "%s" % open(pjoin(Pdir, "ajob.no_ps.log")).read()
            else:
                logger.warning(
                    """ %s SubProcesses doesn\'t have available phase-space.
        ...
        if (
            not self.history
            or "survey" in self.history[-1]
            or self.ninitial == 1
            or self.run_card["gridpack"]
        ):
            # will be done during the refine (more precisely in gen_ximprove)
    ...
    def do_refine(self, line):
                ...
                if os.path.exists(pjoin(Pdir, "ajob1")):
                    ...
                    for i, job in enumerate(alljobs):
                        ...
                        self.launch_job(
                            "%s" % job,
                            cwd=Pdir,
                            remaining=(nb_tot - i - 1),
                            run_type="Refine number %s on %s (%s/%s)"
                            % (self.nb_refine, subdir, nb_proc + 1, len(subproc)),
                        )

        self.monitor(
            run_type="All job submitted for refine number %s" % self.nb_refine,
            html=True,
        )
        ...
        self.results.add_detail(
            "run_statistics", dict(self.results.get_detail("run_statistics"))
        )
    ...
    def do_combine_events(self, line):
        ...
        self.banner.add_generation_info(
            self.results.current["cross"], self.run_card["nevents"]
        )
        if not hasattr(self, "random_orig"):
            self.random_orig = 0
                ...
                if len(AllEvent) >= 80:  # perform a partial unweighting
                    AllEvent.unweight(
                        pjoin(
                            ...
                            "Events",
                            self.run_name,
                            "partials%s.lhe.gz" % partials,
                        ),
                        get_wgt,
                        log_level=5,
                        trunc_error=1e-2,
                        event_target=self.run_card["nevents"],
                    )
                    AllEvent = lhe_parser.MultiEventFile()
                    AllEvent.banner = self.banner
                    AllEvent.add(
        ...
        if len(AllEvent) == 0:
            ...
            nb_event = AllEvent.unweight(
                pjoin(self.me_dir, "Events", self.run_name, "unweighted_events.lhe.gz"),
                get_wgt,
                trunc_error=1e-2,
                event_target=self.run_card["nevents"],
                log_level=logging.DEBUG,
                normalization=self.run_card["event_norm"],
        ...
        if partials:
            for i in range(partials):
                try:
                    ...
                    os.remove(
                        pjoin(
                            self.me_dir, "Events", self.run_name, "partials%s.lhe" % i
                        )
                    )
    ...
    def correct_bias(self):
        ...
        bannerfile = lhe_parser.EventFile(
            pjoin(self.me_dir, "Events", self.run_name, ".banner.tmp.gz"), "w"
        )
    ...
    def do_store_events(self, line):
        ...
        for name in ["events.lhe", "unweighted_events.lhe"]:
            ...
            if os.path.exists(finput):
                logger.debug(
                    "File %s exists BAAAAD. Not move anymore!" % pjoin(E_path, name)
                )
    ...
    def do_create_gridpack(self, line):
        ...
        if not self.run_tag:
            self.run_tag = "tag_1"
        os.system(
            'sed -i.bak "s/ *.false.*=.*GridRun/  .true.  =  GridRun/g" %s/Cards/grid_card.dat'
            % self.me_dir
        )
        misc.call(["./bin/internal/restore_data", self.run_name], cwd=self.me_dir)
        misc.call(
            ["./bin/internal/store4grid", self.run_name, self.run_tag], cwd=self.me_dir
        )
        misc.call(["./bin/internal/clean"], cwd=self.me_dir)
        misc.call(["./bin/internal/make_gridpack"], cwd=self.me_dir)
        files.mv(
            pjoin(self.me_dir, "gridpack.tar.gz"),
            pjoin(self.me_dir, "%s_gridpack.tar.gz" % self.run_name),
        )
        os.system(
            'sed -i.bak "s/\s*.true.*=.*GridRun/  .false.  =  GridRun/g" %s/Cards/grid_card.dat'
            % self.me_dir
        )
    ...
    def do_shower(self, line):
        ...
        if len(args) > 1 and args[0] in self._interfaced_showers:
            ...
            chosen_showers = [
                sorted(
                    chosen_showers,
                    key=lambda sh: shower_priority.index(sh)
                    if sh in shower_priority
                    else len(shower_priority) + 1,
                )[0]
            ]

        for shower in chosen_showers:
            self.exec_cmd(
                "%s %s" % (shower, " ".join(args)), postcmd=False, printcmd=False
            )
    ...
    """
        ...
        try:
            p = subprocess.Popen(
                ["./get_pythia8_version.py", py8_path],
    ...
    def setup_Pythia8RunAndCard(self, PY8_Card, run_type):
        ...
        if PY8_Card["HEPMCoutput:file"] in ["auto", "autoremove"]:
            ...
            HepMC_event_output = pjoin(
                self.me_dir, "Events", self.run_name, "%s_pythia8_events.hepmc" % tag
            )
            PY8_Card.MadGraphSet(
                "HEPMCoutput:file", "%s_pythia8_events.hepmc" % tag, force=True
            )
            ...
            if len(fifo_specs) <= 1:
                fifo_path = pjoin(
                    self.me_dir, "Events", self.run_name, "PY8.hepmc.fifo"
                )
                ...
                if os.path.exists(fifo_path):
                    if stat.S_ISFIFO(os.stat(fifo_path).st_mode):
                        logger.warning(
                            "PY8 will be reusing already existing "
                            + "custom fifo file at:\n  %s" % fifo_path
                        )
                    else:
                        raise InvalidCmd(
   ...
   %s
already exists and is not a fifo file."""
                            % fifo_path
                        )
                else:
                    misc.mkfifo(fifo_path)
            ...
            HepMC_event_output = pjoin(
                self.me_dir, "Events", self.run_name, PY8_Card["HEPMCoutput:file"]
            )
        ...
        if run_type == "MLM":
            ...
            if PY8_Card["JetMatching:qCut"] == -1.0:
                PY8_Card.MadGraphSet(
                    "JetMatching:qCut", 1.5 * self.run_card["xqcut"], force=True
                )

            if PY8_Card["JetMatching:qCut"] < (1.5 * self.run_card["xqcut"]):
                logger.error(
                    "The MLM merging qCut parameter you chose (%f) is less than "
                    % PY8_Card["JetMatching:qCut"]
            ...
            if PY8_Card["SysCalc:qWeed"] == -1.0:
                PY8_Card.MadGraphSet(
                    "SysCalc:qWeed", self.run_card["xqcut"], force=True
                )
                ...
                if self.run_card["use_syst"]:
                    if self.run_card["sys_matchscale"] == "auto":
                        ...
                        value = [
                            ...
                            if factor * qcut > 1.5 * self.run_card["xqcut"]
                        ]
                        PY8_Card.MadGraphSet("SysCalc:qCutList", value, force=True)
                    else:
                        qCutList = [
                            float(qc) for qc in self.run_card["sys_matchscale"].split()
                        ]
            ...
            for scale in PY8_Card["SysCalc:qCutList"]:
                if scale < (1.5 * self.run_card["xqcut"]):
                    logger.error(
                        "One of the MLM merging qCut parameter you chose (%f) in the variation list"
                        % scale
            ...
            if PY8_Card["JetMatching:nJetMax".lower()] == -1:
                logger.info(
                    "No user-defined value for Pythia8 parameter "
        ...
        # We use the positivity of 'ktdurham' cut as a CKKWl marker.
            ...
            if PY8_Card["Merging:Process"] == "<set_by_user>":
                raise self.InvalidCmd(
                    "When running CKKWl merging, the user must"
            ...
            if self.run_card["ptlund"] <= 0.0 and self.run_card["ktdurham"] > 0.0:
                ...
                PY8_Card.subruns[0].MadGraphSet(
                    "Merging:Dparameter", self.run_card["dparameter"]
                )
                CKKW_cut = "ktdurham"
            ...
            if PY8_Card["Merging:TMS"] == -1.0:
                if self.run_card[CKKW_cut] > 0.0:
                    PY8_Card.MadGraphSet(
                        "Merging:TMS", self.run_card[CKKW_cut], force=True
                    )
                else:
                    raise self.InvalidCmd(
                        "When running CKKWl merging, the user"
                        + " select a '%s' cut larger than 0.0 in the run_card."
                        % CKKW_cut
                    )
            if PY8_Card["Merging:TMS"] < self.run_card[CKKW_cut]:
                logger.error(
                    "The CKKWl merging scale you chose (%f) is less than "
                    % PY8_Card["Merging:TMS"]
                    + "the %s cut specified in the run_card parameter (=%f).\n"
                    % (CKKW_cut, self.run_card[CKKW_cut])
                    + "It is incorrect to use a smaller CKKWl scale than the generation-level %s cut!"
                    % CKKW_cut
                )
            ...
            if PY8_Card["SysCalc:tmsList"] == "auto":
                if self.run_card["use_syst"]:
                    if self.run_card["sys_matchscale"] == "auto":
                        ...
                        tmsList = [
                            float(tms)
                            for tms in self.run_card["sys_matchscale"].split()
                        ]
            ...
            for scale in PY8_Card["SysCalc:tmsList"]:
                if scale < self.run_card[CKKW_cut]:
                    logger.error(
                        "One of the CKKWl merging scale you chose (%f) in the variation list"
                        % scale
                        + " (either via 'SysCalc:tmsList' in the PY8 shower card or "
                        + "'sys_matchscale' in the run_card) is less than %f, "
                        % self.run_card[CKKW_cut]
    ...
    def do_pythia8(self, line):
        ...
        if not no_default:
            self.ask_pythia_run_configuration(
                args[-1], pythia_version=8, banner=self.banner
            )
        ...
        if self.run_card["event_norm"] not in ["unit", "average"]:
            logger.critical(
                "Pythia8 does not support normalization to the sum. Not running Pythia8"
            )
            return
            # \n"+\
        ...
        if not self.options["mg5amc_py8_interface_path"] or not os.path.exists(
            pjoin(self.options["mg5amc_py8_interface_path"], "MG5aMC_PY8_interface")
        ):
            raise self.InvalidCmd(
...
Please install this tool with the following MG5_aMC command:
  MG5_aMC> install mg5amc_py8_interface_path"""
            ...
            pythia_main = pjoin(
                self.options["mg5amc_py8_interface_path"], "MG5aMC_PY8_interface"
            )
        ...
        self.update_status(
            "\033[92mRunning Pythia8 [arXiv:1410.3012]\033[0m", "pythia8"
        )
        ...
        # Now write the card.
        pythia_cmd_card = pjoin(
            self.me_dir, "Events", self.run_name, "%s_pythia8.cmd" % tag
        )
...
!
"""
            % (preamble + pythia_main, os.path.basename(pythia_cmd_card))
            + cmd_card.getvalue()
        )

        # launch pythia8
        pythia_log = pjoin(self.me_dir, "Events", self.run_name, "%s_pythia8.log" % tag)
        ...
        if os.path.exists("/usr/bin/env"):
            ...
            if not shell_exe:
                raise self.InvalidCmd(
                    "No s hell could be found in your environment.\n"
                    + "Make sure that either '%s' is in your path or that the" % shell
                    + " command '/usr/bin/env %s' exists and returns a valid path."
                    % shell
                )

        exe_cmd = "#!%s\n%s" % (
            shell_exe,
            " ".join([preamble + pythia_main, os.path.basename(pythia_cmd_card)]),
        )
        ...
        # it proceed.
        is_HepMC_output_fifo = (
            False
            if not HepMC_event_output
            else (
                os.path.exists(HepMC_event_output)
                and stat.S_ISFIFO(os.stat(HepMC_event_output).st_mode)
            )
        )
...
You can follow PY8 run with the following command (in a separate terminal):
    tail -f %s"""
            ...
            if not no_default:
                logger.info(
                    "You can now run a tool that reads the following fifo file:"
            ...
            if self.options["run_mode"] != 0:
                ...
                lhe_file = lhe_parser.EventFile(
                    pjoin(
                        self.me_dir,
                        "Events",
                        self.run_name,
                        PY8_Card.subruns[0]["Beams:LHEF"],
                    )
                )
                ...
                if PY8_Card["Main:numberOfEvents"] == -1:
                    ...
                    if n_events > n_available_events:
                        raise self.InvalidCmd, "You specified more events (%d) in the PY8 parameter" % n_events + "'Main:numberOfEvents' than the total number of events available (%d)" % n_available_events + " in the event file:\n %s" % pjoin(
                            self.me_dir,
                            "Events",
                            self.run_name,
                            PY8_Card.subruns[0]["Beams:LHEF"],
            ...
            if self.options["run_mode"] == 0 or (
                self.options["run_mode"] == 2 and self.options["nb_core"] == 1
            ):
                # No need for parallelization anymore
                self.cluster = None
                logger.info(
                    "Follow Pythia8 shower by running the "
                    + "following command (in a separate terminal):\n    tail -f %s"
                    % pythia_log
                )

                if self.options["run_mode"] == 2 and self.options["nb_core"] > 1:
                    ret_code = self.cluster.launch_and_wait(
                        wrapper_path,
                        argument=[],
                        stdout=pythia_log,
                ...
                if self.run_card["event_norm"] == "sum":
                    ...
                    raise InvalidCmd(
                        "Pythia8 parallelization with event_norm set to 'sum' is not supported."
                        "Either run in single core or change event_norm to 'average'."
                    )

                # Create the parallelization folder
                parallelization_dir = pjoin(
                    self.me_dir, "Events", self.run_name, "PY8_parallelization"
                )
...
"""
                ...
                os.system(
                    "sed -i 's/49001010/4900101/g' Events/%s/unweighted_events.lhe"
                    % self.run_name
                )
                os.system(
                    "sed -i 's/49001011/4900101/g' Events/%s/unweighted_events.lhe"
                    % self.run_name
                )
                os.system(
                    "sed -i 's/49001012/4900101/g' Events/%s/unweighted_events.lhe"
                    % self.run_name
                )
                os.system(
                    "sed -i 's/49001013/4900101/g' Events/%s/unweighted_events.lhe"
                    % self.run_name
                )
                os.system(
                    "sed -i 's/49001014/4900101/g' Events/%s/unweighted_events.lhe"
                    % self.run_name
                )
                os.system("gzip Events/%s/unweighted_events.lhe" % self.run_name)

                n_splits = lhe_file.split(
                    partition=partition, cwd=parallelization_dir, zip=True
                )

                if n_splits != len(partition):
                    raise MadGraph5Error(
                        "Error during lhe file splitting. Expected %d files but obtained %d."
                ...
                for split_id in range(n_splits):
                    ...
                    shutil.move(
                        pjoin(
                            parallelization_dir, lhe_file.name + "_%d.lhe.gz" % split_id
                        ),
                ...
                for i, split_file in enumerate(split_files):
                    ...
                    split_PY8_Card = banner_mod.PY8Card(
                        pjoin(parallelization_dir, "PY8Card.dat")
                    )
                    # Make sure to sure the number of split_events determined during the splitting.
                    split_PY8_Card.systemSet(
                        "Main:numberOfEvents", partition_for_PY8[i]
                    )
                    split_PY8_Card.systemSet(
                        "HEPMCoutput:scaling",
                        split_PY8_Card["HEPMCoutput:scaling"]
                    ...
                    split_PY8_Card.write(
                        pjoin(parallelization_dir, "PY8Card_%d.dat" % i),
                        pjoin(parallelization_dir, "PY8Card.dat"),
                        add_missing=False,
                    )
                    in_files = [
                        pjoin(parallelization_dir, os.path.basename(pythia_main)),
                        pjoin(parallelization_dir, "PY8Card_%d.dat" % i),
                    ...
                    if self.options["cluster_temp_path"] is None:
                        ...
                        for in_file in in_files + [
                            pjoin(parallelization_dir, "run_PY8.sh")
                        ]:
                ...
                def wait_monitoring(Idle, Running, Done):
                    if Idle + Running + Done == 0:
                        return
                    logger.info(
                        "Pythia8 shower jobs: %d Idle, %d Running, %d Done [%s]"
                        % (
                            Idle,
                            Running,
                            Done,
                            misc.format_time(time.time() - startPY8timer),
                        )
                    )
                ...
                if self.options["cluster_temp_path"]:
                    ...
                    for i, split_file in enumerate(split_files):
                        misc.call(
                            ["tar", "-xzf", "split_%d.tar.gz" % i],
                            cwd=parallelization_dir,
                        )
                ...
                for split_dir in split_dirs:
                    ...
                    pythia_log_file.write(
                        " -> Pythia8 log file for run %d <-" % i + "\n"
                    )
                    pythia_log_file.write("=" * 35 + "\n")
                    ...
                    if len(xsecs) > 0:
                        ...
                        if len(PY8_extracted_information["cross_sections"]) == 0:
                            ...
                            for key, value in xsecs.items():
                                PY8_extracted_information["cross_sections"][key][
                                    0
                                ] += value[0]
                ...
                if not djr_HwU is None:
                    djr_HwU.output(
                        pjoin(self.me_dir, "Events", self.run_name, "djrs"),
                        format="HwU",
                    )
                    shutil.move(
                        pjoin(self.me_dir, "Events", self.run_name, "djrs.HwU"),
                        pjoin(
                            self.me_dir, "Events", self.run_name, "%s_djrs.dat" % tag
                        ),
                    )
                ...
                if n_added > 0:
                    for key in PY8_extracted_information["cross_sections"]:
                        ...
                        PY8_extracted_information["cross_sections"][key][1] = math.sqrt(
                            PY8_extracted_information["cross_sections"][key][1]
                        ) / float(n_added)
                ...
                if not pts_HwU is None:
                    pts_HwU.output(
                        pjoin(self.me_dir, "Events", self.run_name, "pts"), format="HwU"
                    )
                    shutil.move(
                        pjoin(self.me_dir, "Events", self.run_name, "pts.HwU"),
                ...
                if len(all_hepmc_files) > 0:
                    hepmc_output = pjoin(
                        self.me_dir, "Events", self.run_name, HepMC_event_output
                    )
                    with misc.TMP_directory() as tmp_dir:
                        ...
                        for hepmc_file in all_hepmc_files:
                            ...
                            if pid == 0:
                                misc.call(
                                    "head -n -1 %s | tail -n +%d > %s/tmpfile"
                                    % (hepmc_file, n_head, os.path.dirname(hepmc_file)),
                                    shell=True,
                                )
                                misc.call(
                                    ["mv", "tmpfile", os.path.basename(hepmc_file)],
                                    cwd=os.path.dirname(hepmc_file),
                                )
                                ...
                                os.system(
                                    " ".join(
                                        ["sed", "-i"]
                                        + ["-e '%id'" % (i + 1) for i in range(n_head)]
                                        + ["-e '$d'", hepmc_file]
                                    )
                                )
                        ...
                        os.system(
                            " ".join(
                                ["cat", pjoin(tmp_dir, "header.hepmc")]
                                + all_hepmc_files
        ...
        if not os.path.isfile(
            pythia_log
        ) or "Inclusive cross section:" not in "\n".join(
            open(pythia_log, "r").readlines()[-20:]
        ):
            logger.warning(
        ...
        if run_type in merged_run_types:
            # From the log file
            if all(
                PY8_extracted_information[_] is None
                for _ in ["sigma_m", "Nacc", "Ntry"]
            ):
                ...
                if self.options["run_mode"] == 0 or (
                    ...
                    (
                        PY8_extracted_information["sigma_m"],
                        PY8_extracted_information["Nacc"],
                        PY8_extracted_information["Ntry"],
                    ) = self.parse_PY8_log_file(
                        pjoin(
                            self.me_dir, "Events", self.run_name, "%s_pythia8.log" % tag
                        )
                    )
                else:
                    logger.warning(
            ...
            if not any(
                ...
                self.results.add_detail(
                    "cross_pythia", PY8_extracted_information["sigma_m"]
                )
                self.results.add_detail(
                    "nb_event_pythia", PY8_extracted_information["Nacc"]
                )
                # Shorthands
            ...
            if (
                os.path.isfile(djr_output)
                and len(PY8_extracted_information["cross_sections"]) == 0
            ):
                # When parallelization is enable we shouldn't have cannot look in the log in this way
                if self.options["run_mode"] == 0 or (
                    self.options["run_mode"] == 2 and self.options["nb_core"] == 1
                    ...
                    PY8_extracted_information[
                        "cross_sections"
                    ] = self.extract_cross_sections_from_DJR(djr_output)
                else:
                    logger.warning(
            ...
            if cross_sections:
                ...
                cross_sections = dict(
                    ...
                    for xsec, value in cross_sections.items()
                    if not central_merging_re.match(xsec) is None
                )
                central_scale = (
                    PY8_Card["JetMatching:qCut"]
                    if int(self.run_card["ickkw"]) == 1
                    else PY8_Card["Merging:TMS"]
                )
                if central_scale in cross_sections:
                    self.results.add_detail(
                        "cross_pythia8", cross_sections[central_scale][0]
                    )
                    self.results.add_detail(
                        "error_pythia8", cross_sections[central_scale][1]
                    )
            ...
            xsecs_file = open(
                pjoin(
                    self.me_dir, "Events", self.run_name, "%s_merged_xsecs.txt" % tag
                ),
                "w",
            )
            if cross_sections:
                ...
                xsecs_file.write(
                    "Cross-sections could not be read from the"
        ...
        if int(self.run_card["ickkw"]):
            ...
            if "MGGenerationInfo" in self.banner:
                self.banner["MGGenerationInfo"] += (
                    "#  Matched Integrated weight (pb)  :  %s\n"
        ...
        self.update_status(
            "Pythia8 shower finished after %s."
            % misc.format_time(time.time() - startPY8timer),
            level="pythia8",
        )
    ...
    def parse_PY8_log_file(self, log_file_path):
        ...
        pythiare = re.compile(
            "Les Houches User Process\(es\)\s*\d+\s*\|\s*(?P<tried>\d+)\s*(?P<selected>\d+)\s*(?P<generated>\d+)\s*\|\s*(?P<xsec>[\d\.e\-\+]+)\s*(?P<xsec_error>[\d\.e\-\+]+)"
        )
        pythia_xsec_re = re.compile(
            "Inclusive cross section\s*:\s*(?P<xsec>[\d\.e\-\+]+)\s*(?P<xsec_error>[\d\.e\-\+]+)"
        )
        ...
        for line in misc.BackRead(log_file_path):
            ...
            if not info:
                ...
                try:
                    ...
                    if Nacc == 0:
                        raise self.InvalidCmd, "Pythia8 shower failed since it" + " did not accept any event from the MG5aMC event file."
                    return sigma_m, Nacc, Ntry
                except ValueError:
                    return None, None, None

        raise self.InvalidCmd, "Could not find cross-section and event number information " + "in Pythia8 log\n  '%s'." % log_file_path
    ...
    def extract_cross_sections_from_DJR(self, djr_output):
        ...
        all_nodes = dict((int(node.getAttribute("id")), node) for node in run_nodes)
        try:
    ...
    def do_pythia(self, line):
        ...
        if self.run_card["event_norm"] != "sum":
            logger.error(
                "pythia-pgs require event_norm to be on sum. Do not run pythia6"
            )
            return
        ...
        if not re.search(
            r"^\s*LHAPATH=%s/PDFsets" % pythia_src,
        ...
        self.cluster.launch_and_wait(
            pjoin(pythia_src, "pythia"),
            input_files=[
        ...
        if int(self.run_card["ickkw"]):
            ...
            pythia_log = misc.BackRead(
                pjoin(self.me_dir, "Events", self.run_name, "%s_pythia.log" % tag)
            )
            pythiare = re.compile(
                "\s*I\s+0 All included subprocesses\s+I\s+(?P<generated>\d+)\s+(?P<tried>\d+)\s+I\s+(?P<xsec>[\d\.D\-+]+)\s+I"
            )
            for line in pythia_log:
                ...
                try:
                    ...
                    # compute pythia error
                    error = self.results[self.run_name].return_tag(self.run_tag)[
                        "error"
                    ]
                    if Nacc:
                        error_m = math.sqrt(
        ...
        if int(self.run_card["ickkw"]):
            misc.gzip(
                pjoin(self.me_dir, "Events", "beforeveto.tree"),
                stdout=pjoin(
                    self.me_dir,
                    "Events",
                    self.run_name,
                    tag + "_pythia_beforeveto.tree.gz",
                ),
            )
        ...
        if self.run_card["use_syst"] in self.true:
            ...
            try:
                ...
                if os.path.exists(pjoin(self.me_dir, "Events", "syst.dat")):
                    # Store syst.dat
                    misc.gzip(
                        pjoin(self.me_dir, "Events", "syst.dat"),
                        stdout=pjoin(
                            self.me_dir,
                            "Events",
                            self.run_name,
                            tag + "_pythia_syst.dat.gz",
                        ),
                    )
                    ...
                    if os.path.exists(pjoin(self.me_dir, "Events", "syscalc.dat")):
                        ...
                        misc.gzip(
                            pjoin(self.me_dir, "Events", "syscalc.dat"),
    ...
    def do_remove(self, line):
        ...
        if run == "all":
            ...
            if os.path.exists(pjoin(self.me_dir, "Events", "all")):
                logger.warning(
                    "A run with name all exists. So we will not supress all processes."
                )
            else:
                for match in misc.glob(
                    pjoin("*", "*_banner.txt"), pjoin(self.me_dir, "Events")
                ):
        ...
        if tag:
            ...
            if "parton" in mode or "all" in mode:
                try:
                    ...
                    if os.path.exists(
                        pjoin(self.me_dir, "Events", run, "unweighted_events.lhe.gz")
                    ):
                        to_delete.append("unweighted_events.lhe.gz")
                    if os.path.exists(
                        pjoin(self.me_dir, "HTML", run, "plots_parton.html")
                    ):
                        to_delete.append(
                            pjoin(self.me_dir, "HTML", run, "plots_parton.html")
                        )
                    if nb_rm != len(to_delete):
                        logger.warning(
                            "Be carefull that partonic information are on the point to be removed."
                        )
        if "all" in mode:
            ...
            if "parton" not in mode:
                to_delete = [
                    f
                    for f in to_delete
                    if "delphes" in f or "pgs" in f or "pythia" in f
                ]
        ...
        if "all" in mode or "channel" in mode:
            try:
                ...
                to_delete += misc.glob(
                    pjoin("*", "%s*" % run), pjoin(self.me_dir, "SubProcesses")
                )
                to_delete += misc.glob(
                    pjoin("*", "*", "%s*" % run), pjoin(self.me_dir, "SubProcesses")
                )
        ...
        if "banner" in mode:
            ...
            if tag:
                ...
                try:
                    os.remove(
                        pjoin(
                            self.me_dir, "Events", run, "%s_%s_banner.txt" % (run, tag)
                        )
                    )
                except Exception:
            ...
            logger.info(
                """The banner is not removed. In order to remove it run:
    remove %s all banner %s"""
                % (run, tag and "--tag=%s " % tag or "")
            )
    ...
    def do_syscalc(self, line):
        ...
        if any([arg in ["all", "parton"] for arg in args]):
            ...
            if os.path.exists(filename):
                shutil.move(
                    filename, pjoin(self.me_dir, "Events", "unweighted_events.lhe")
                )
                self.run_syscalc("parton")
                misc.gzip(
                    pjoin(self.me_dir, "Events", "unweighted_events.lhe"),
        ...
        if any([arg in ["all", "pythia"] for arg in args]):
            ...
            if os.path.exists(filename):
                ...
                misc.gzip(pjoin(self.me_dir, "Events", "syscalc.dat"), stdout=filename)
            else:
    ...
    def store_result(self):
        ...
        tag = self.run_card["run_tag"]
        self.update_status("storing files of previous run", level=None, error=True)
        ...
        if "pythia" in self.to_store:
            self.update_status(
                "Storing Pythia files of previous run", level="pythia", error=True
            )
        ...
        if "pythia8" in self.to_store:
            ...
            if os.path.isfile(file_path):
                if "nopy8" in self.to_store:
                    ...
                    self.update_status(
                        "Storing Pythia8 files of previous run",
                        level="pythia",
                        error=True,
                    )
    ...
    def launch_job(
        ...
        **opt
        ...
        elif (cwd and os.path.exists(pjoin(cwd, exe))) and not os.access(
            pjoin(cwd, exe), os.X_OK
        ):
            os.system("chmod +x %s " % pjoin(cwd, exe))

        if mode == 0:
            self.update_status(
                (remaining, 1, self.total_jobs - remaining - 1, run_type),
                level=None,
                force=False,
            )
            start = time.time()
            ...
            if "ajob" in exename:
                ...
                self.cluster.submit(
                    exe, argument=argument, stdout=stdout, cwd=cwd, **opt
                )
    ...
    def monitor(self, run_type="monitor", mode=None, html=False):
        ...
        if mode > 0:
            if html:
                update_status = lambda idle, run, finish: self.update_status(
                    (idle, run, finish, run_type),
                    level=None,
                    force=False,
                    starttime=starttime,
                )
                update_first = lambda idle, run, finish: self.update_status(
                    (idle, run, finish, run_type),
                    level=None,
                    force=True,
                    starttime=starttime,
            ...
            try:
                ...
                if not self.force:
                    ans = self.ask(
                        'Cluster Error detected. Do you want to clean the queue? ("c"=continue the run anyway)',
                        default="y",
                        choices=["y", "n", "c"],
                    )
                else:
    ...
    def configure_directory(self, html_opening=True):
        ...
        time_mod = max(
            [
                os.path.getmtime(pjoin(self.me_dir, "Cards", "run_card.dat")),
        ...
        if (
            ...
            and hasattr(self, "random")
            and hasattr(self, "run_card")
        ):
        ...
        if os.path.exists(
            pjoin(self.me_dir, "Source", "BIAS", bias_name, "bias_dependencies")
        ):
            files.ln(
                pjoin(self.me_dir, "Source", "BIAS", bias_name, "bias_dependencies"),
        ...
        if self.proc_characteristics["bias_module"] != bias_name and os.path.isfile(
            pjoin(self.me_dir, "lib", "libbias.a")
        ):
        ...
        if self.run_card["bias_module"] != "dummy":
            ...
            if (
                "VALID" not in bias_module_valid.upper()
                or "INVALID" in bias_module_valid.upper()
            ):
                raise InvalidCmd(
                    "The bias module '%s' cannot be used because of:\n%s"
    ...
    def get_Gdir(self, Pdir=None, symfact=None):
            ...
            if self.me_dir in self.Gdirs[0]:
                if Pdir is None:
                    if not symfact:
                        ...
                        return (
                            list(itertools.chain(*self.Gdirs[0].values())),
                            self.Gdirs[1],
                        )
                else:
                    if not symfact:
        ...
        for P in Pdirs:
            ...
            for line in open(
                pjoin(self.me_dir, "SubProcesses", os.path.basename(P), "symfact.dat")
            ):
    ...
    ############################################################################
    def set_run_name(
        self, name, tag=None, level="parton", reload_card=False, allow_new_tag=True
    ):
    ...
    def check_nb_events(self):
        ...
        if nb_event > 1000000:
            ...
            logger.warning(
                "Limiting number to 1M. Use multi_run for larger statistics."
            )
            path = pjoin(self.me_dir, "Cards", "run_card.dat")
            os.system(
                r"""perl -p -i.bak -e "s/\d+\s*=\s*nevents/1000000 = nevents/" %s"""
                % path
            )
    ...
    def update_random(self):
        ...
        if self.random > 30081 * 30081:  # can't use too big random number
            raise MadGraph5Error, "Random seed too large " + str(
                self.random
            ) + " > 30081*30081"
    ...
    def treat_CKKW_matching(self):
        ...
        translation = {"e1": e1, "e2": e2, "pd": pd, "lha": lha, "xq": xq}
    ...
    def create_root_file(
        self, input="unweighted_events.lhe", output="unweighted_events.root"
    ):
        ...
        try:
            misc.call(
                ["%s/ExRootLHEFConverter" % eradir, input, output],
                cwd=pjoin(self.me_dir, "Events"),
            )
        except Exception:
    ...
    def run_syscalc(self, mode="parton", event_path=None, output=None):
        ...
        if "&&" in self.run_card["sys_pdf"]:
            ...
            lhaid += [
                l
                for l in self.run_card["sys_pdf"].split()
                if not l.isdigit() or int(l) > 500
            ]
        try:
            ...
            logger.warning(
                "Systematic computation requires lhapdf to run. Bypass SysCalc"
            )
            return
        ...
        to_syscalc = {
            "sys_scalefact": self.run_card["sys_scalefact"],
        ...
        template = open(
            pjoin(self.me_dir, "bin", "internal", "syscalc_template.dat")
        ).read()

        if "&&" in to_syscalc["sys_pdf"]:
            to_syscalc["sys_pdf"] = (
                to_syscalc["sys_pdf"].split("#", 1)[0].replace("&&", " \n ")
            )
        else:
        ...
        if not event_path:
            if mode == "parton":
                ...
                if not (
                    os.path.exists(event_path) or os.path.exists(event_path + ".gz")
                ):
                    event_path = pjoin(event_dir, "unweighted_events.lhe")
                output = pjoin(event_dir, "syscalc.lhe")
                stdout = open(
                    pjoin(event_dir, self.run_name, "%s_systematics.log" % (mode)), "w"
                )
            elif mode == "Pythia":
                stdout = open(
                    pjoin(event_dir, self.run_name, "%s_%s_syscalc.log" % (tag, mode)),
                    "w",
                )
        ...
        self.update_status(
            "Calculating systematics for %s level" % mode, level=mode.lower()
        )
        try:
            proc = misc.call(
                [os.path.join(scdir, "sys_calc"), event_path, card, output],
                stdout=stdout,
                stderr=subprocess.STDOUT,
                cwd=event_dir,
            )
            ...
            logger.error(
                "fail to run syscalc: %s. Please check that SysCalc is correctly installed."
                % error
            )
        else:
        ...
        self.update_status(
            "End syscalc for %s level" % mode, level=mode.lower(), makehtml=False
        )
    ...
    def ask_run_configuration(self, mode=None, args=[]):
        ...
        switch, cmd_switch = self.ask(
            "",
            "0",
            [],
            ask_class=self.action_switcher,
            mode=mode,
            line_args=args,
            force=self.force,
        ...
        if (
            switch["analysis"].upper() in ["MADANALYSIS5"]
            and not switch["shower"] == "OFF"
        ):
    ...
    0 / auto    : running existing cards\n"""
        ...
        if mode == "auto":
            auto = True
            if pythia_version == 6 and os.path.exists(
        ...
        if auto:
            self.ask_edit_cards(
                cards,
                from_banner=["param", "run"],
                mode="auto",
                plot=(pythia_version == 6),
                banner=banner,
            )
        else:
            self.ask_edit_cards(
...
class GridPackCmd(MadEventCmd):
    ...
    def __init__(self, me_dir=None, nb_event=0, seed=0, gran=-1, *completekey, **stdin):
        ...
        if me_dir and nb_event and seed:
            ...
            raise MadGraph5Error, "Gridpack run failed: " + str(me_dir) + str(
                nb_event
            ) + str(seed)
    ...
    def get_Pdir(self):
        ...
        if not self.readonly:
            self.Pdirs = [
                pjoin(self.me_dir, "SubProcesses", l.strip())
    ...
    def launch(self, nb_event, seed):
        ...
        if not self.readonly:
            ...
            misc.call(
                [pjoin(self.me_dir, "bin", "internal", "restore_data"), "default"],
                cwd=self.me_dir,
            )
    ...
    def refine4grid(self, nb_event):
        ...
        self.gscalefact = (
            x_improve.gscalefact
        )  # store jacobian associate to the gridpack
        ...
        subproc = [
            P
            for P in os.listdir(pjoin(self.me_dir, "SubProcesses"))
            if P.startswith("P")
            and os.path.isdir(pjoin(self.me_dir, "SubProcesses", P))
        ]
        devnull = open(os.devnull, "w")
        for nb_proc, subdir in enumerate(subproc):
            subdir = subdir.strip()
    ...
    def do_combine_events(self, line):
        ...
        self.banner.write(
            pjoin(outdir, self.run_name, "%s_%s_banner.txt" % (self.run_name, tag))
        )
        ...
        nb_event = AllEvent.unweight(
            pjoin(outdir, self.run_name, "unweighted_events.lhe.gz"),
            get_wgt,
            trunc_error=1e-2,
            event_target=self.nb_event,
            log_level=logging.DEBUG,
...
class MadLoopInitializer(object):
    ...
    def make_and_run(dir_name, checkRam=False):
        ...
        if not checkRam:
            ...
            retcode = subprocess.call(
                "./check", cwd=dir_name, stdout=devnull, stderr=devnull
            )

            run_time = time.time() - start
            ram_usage = None
        else:
            ptimer = misc.ProcessTimer(
                ["./check"],
                cwd=dir_name,
                shell=False,
                stdout=devnull,
    ...
    @staticmethod
    def fix_PSPoint_in_check(
        dir_path, read_ps=True, npoints=1, hel_config=-1, mu_r=0.0, split_orders=-1
    ):
        ...
        if (
            ...
            or not os.path.basename(dir_path) == "check_sa.f"
        ):
            file_path = pjoin(dir_path, "check_sa.f")
            if not os.path.isfile(file_path):
                directories = [
                    d
                    for d in misc.glob("P*_*", dir_path)
        ...
        if not os.path.isfile(file_path):
            raise MadGraph5Error(
                "Could not find the location of check_sa.f"
        ...
        check_sa = re.sub(
            r"READPS = \S+\)",
            "READPS = %s)" % (".TRUE." if read_ps else ".FALSE."),
            check_sa,
        )
        check_sa = re.sub(r"NPSPOINTS = \d+", "NPSPOINTS = %d" % npoints, check_sa)
        if hel_config != -1:
            check_sa = re.sub(
                r"SLOOPMATRIX\S+\(\S+,MATELEM,",
                "SLOOPMATRIXHEL_THRES(P,%d,MATELEM," % hel_config,
                check_sa,
            )
        else:
            check_sa = re.sub(
                r"SLOOPMATRIX\S+\(\S+,MATELEM,",
                "SLOOPMATRIX_THRES(P,MATELEM,",
                check_sa,
            )
        if mu_r > 0.0:
            check_sa = re.sub(
                r"MU_R=SQRTS",
                "MU_R=%s" % (("%.17e" % mu_r).replace("e", "d")),
                check_sa,
            )
        elif mu_r < 0.0:
        ...
        if split_orders > 0:
            check_sa = re.sub(
                r"SET_COUPLINGORDERS_TARGET\(-?\d+\)",
                "SET_COUPLINGORDERS_TARGET(%d)" % split_orders,
                check_sa,
            )
    ...
    def run_initialization(
        ...
        if SubProc_dir is None and run_dir is None:
            raise MadGraph5Error, "At least one of [SubProc_dir,run_dir] must" + " be provided in run_initialization."
        ...
        if run_dir is None:
            directories = [
                dir for dir in misc.glob("P[0-9]*", SubProc_dir) if os.path.isdir(dir)
            ]
            if directories:
                run_dir = directories[0]
            else:
                raise MadGraph5Error, "Could not find a valid running directory" + " in %s." % str(
                    SubProc_dir
                )
        ...
        if not os.path.isfile(MLCardPath):
            raise MadGraph5Error, "Could not find MadLoopParams.dat at %s." % MLCardPath
        else:
        ...
        def need_init():
            ...
            return (
                any(
                    [
                        not os.path.exists(
                            pjoin(run_dir, "MadLoop5_resources", proc_prefix + fname)
                        )
                        for fname in my_req_files
                    ]
                )
                or not os.path.isfile(pjoin(run_dir, "check"))
                or not os.access(pjoin(run_dir, "check"), os.X_OK)
            )
        ...
        while to_attempt != [] and need_init():
            ...
            MadLoopInitializer.fix_PSPoint_in_check(
                run_dir, read_ps=False, npoints=curr_attempt
            )
            compile_time, run_time, ram_usage = MadLoopInitializer.make_and_run(run_dir)
            ...
            if (
                "Process_compilation" not in infos.keys()
                or infos["Process_compilation"] == None
            ):
    ...
    def need_MadLoopInit(proc_dir, subproc_prefix="PV"):
        ...
        def need_init(ML_resources_path, proc_prefix, r_files):
            ...
            return any(
                [
                    not os.path.exists(pjoin(ML_resources_path, proc_prefix + fname))
        ...
        for v_folder in glob.iglob(
            pjoin(proc_dir, "SubProcesses", "%s*" % subproc_prefix)
        ):
            ...
            proc_prefix_file.close()
            if need_init(
                pjoin(proc_dir, "SubProcesses", "MadLoop5_resources"),
                proc_prefix,
                req_files,
            ):
                return True
    ...
    @staticmethod
    def init_MadLoop(
        proc_dir, n_PS=None, subproc_prefix="PV", MG_options=None, interface=None
    ):
        """Advanced commands: Compiles and run MadLoop on RAMBO random PS points to initilize the
        filters."""

        logger.debug(
            "Compiling Source materials necessary for MadLoop " + "initialization."
        )
        ...
        logger.info(
            "Initializing MadLoop loop-induced matrix elements "
        ...
        def wait_monitoring(Idle, Running, Done):
            if Idle + Running + Done == 0:
                return
            logger.debug(
                "MadLoop initialization jobs: %d Idle, %d Running, %d Done"
                % (Idle, Running, Done)
            )
        ...
        VirtualFolders = [
            f
            for f in glob.iglob(pjoin(proc_dir, "SubProcesses", "%s*" % subproc_prefix))
            if (os.path.isdir(f) or os.path.isfile(pjoin(f, "loop_matrix.f")))
        ]
        logger.debug(
            "Now Initializing MadLoop matrix element in %d folder%s:"
            % (len(VirtualFolders), "s" if len(VirtualFolders) > 1 else "")
        )
        logger.debug(
            ", ".join(
                "'%s'" % os.path.basename(v_folder) for v_folder in VirtualFolders
            )
        )
        ...
        for v_folder in VirtualFolders:
            ...
            if init["nPS"] is None:
                raise MadGraph5Error, "Failed the initialization of" + " loop-induced matrix element '%s'%s." % (
                    os.path.basename(v_folder),
                    " (using default n_PS points)"
                    if n_PS is None
                    else " (trying with a maximum of %d PS points)" % (max_mult * n_PS),
                )
            if init["nPS"] == 0:
                logger.debug(
                    "Nothing to be done in '%s', all filters already "
                    % os.path.basename(v_folder)
                    + "present (use the '-r' option to force their recomputation)"
                )
            else:
                logger.debug(
                    "'%s' finished using " % os.path.basename(v_folder)
                    + "%d PS points (%s), in %.3g(compil.) + %.3g(init.) secs."
                    % (
                        abs(init["nPS"]),
                        "DP" if init["nPS"] > 0 else "QP",
                        init["Process_compilation"],
                        init["Initialization"],
                    )
                )
...
if "__main__" == __name__:
    ...
    if not sys.version_info[0] == 2 or sys.version_info[1] < 6:
        sys.exit(
            "MadGraph/MadEvent 5 works only with python 2.6 or later (but not python 3.X).\n"
    ...
    try:
        ...
        logging.config.fileConfig(
            os.path.join(root_path, "internal", "me5_logging.conf")
        )
